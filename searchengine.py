...
---------------------
This program indexes data into a dictionary and searches that index in response to a user query. The matching results
are printed to the terminal and can also be displayed on a webpage if the user chooses to start the server.
"""
...
def create_index(filenames, index, file_titles):
    ...
    {'test1.txt': 'File 1 Title'}
    """
    # reads each file from a list of files
    # for each word in that file, if word not in index creates a new key using that word and list of files it appears in
    # if word is already a key in index, appends the name of that filename to list of files
    ...
    # reads first line of each file and updates file_titles dictionary with key as filename and value as first_line
    for filename in filenames:
        with open(filename) as file:
...
def search(index, query):
    ...
    []
    """
    # for each word in the query, get list of files that has that word, which is the value of the index at that key
    query = query.split()
    posting_list = []

    # for each word in query, set posting_list to the common elems between posting_list and the word's list of files
    for i in range(len(query)):
        word = query[i]
        if i == 0 and word in index:
            posting_list = index[query[0]]
        elif word in index:
            posting_list = common(posting_list, index[word])
        else:
            posting_list = []
